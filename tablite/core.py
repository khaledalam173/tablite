...
from tablite.config import SINGLE_PROCESSING_LIMIT, TEMPDIR, H5_ENCODING, H5_INDEX_FILE
...
class Table(object):
    ...
    def save(self, value):
        ...
        if self._saved != value:
            self._saved = value
            mem.set_saved_flag(self.key, value)
        if self._saved:
            log.info(f"{self} saved in {H5_INDEX_FILE}")

    def __del__(self):
        for key in list(self._columns):
            del self[key]
        mem.delete_table(self.key)
    ...
    def __delitem__(self, key):
        ...
        if isinstance(key, str) and key in self._columns:
            ...
            mem.delete_column_reference(self.key, column_name=key, column_key=col.key)
    ...
    def to_hdf5(self, path):  # Todo -- this wont work.
        """
    ...
    def from_hdf5(self, path):  # Todo -- this wont work.
        """
        imports an exported hdf5 table.

        See to_hdf5 for details
        """
        if isinstance(path, str):
            path = pathlib.Path(path)
    ...
    def import_file(cls, path,  import_as, 
        ...
        for table_key, jsnb in mem.get_imported_tables().items():
            if jsn_str == jsnb:
                return Table.load(key=table_key)  # table already imported.
        # not returned yet? Then it's an import job:
        t = reader(**config)
        mem.set_config(table_key=t.key, config=jsn_str)
        if t.save is False:
    ...
    def filter(self, expressions, filter_type='all'):
        ...
        for task in merge_tasks:
            tmp_true = Table.load(key=task.kwargs['true_key'])
            ...
            tmp_false = Table.load(key=task.kwargs['false_key'])
...
class Column(object):
    def __init__(self, data=None, key=None) -> None:
        ...
        if key is None:
            ...
            length, pages = mem.load_column_attrs(self.key)
            self._len = length
    ...
    def types(self):
        ...
        """
        return mem.get_pages(self.key).get_types()
    ...
    def __getitem__(self, item=None):
        ...
        result = mem.get_data(self.key, slc)
    ...
    def clear(self):
        old_pages = mem.get_pages(self.key)
        self._len = mem.create_virtual_dataset(self.key, pages_before=old_pages, pages_after=[])
    ...
    def insert(self, index, value):        
        ...
        self._len = mem.create_virtual_dataset(self.key, pages_before=old_pages, pages_after=new_pages)
    ...
    def remove(self, value):
        ...
        for ix, page in enumerate(pages):
            ...
            self._len = mem.create_virtual_dataset(self.key, pages_before=pages, pages_after=new_pages)
            return
    ...
    def pop(self, index):
        ...
        if mem.get_ref_count(page) == 1:
            ...
            new_pages = pages[:]
            new_pages[ix] = new_page
        shape = mem.create_virtual_dataset(self.key, pages_before=pages, pages_after=new_pages)
        self._len = shape
        return value
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(key, int):
            ...
            if key.start == key.stop == None and key.step in (None,1): 
                ...
                # example: L[:] = [1,2,3]
                before = mem.get_pages(self.key)
                if isinstance(value, Column):
                    after = mem.get_pages(value.key)
                elif isinstance(value, (list,tuple,np.ndarray)):
                    new_page = Page(value)
                    after = Pages([new_page])
                else:
                    raise TypeError
                self._len = mem.create_virtual_dataset(self.key, pages_before=before, pages_after=after)
                ...
                if isinstance(value, Column):
                    after = before_slice + mem.get_pages(value.key)
                elif isinstance(value, (list, tuple, np.ndarray)):
                    if not before_slice:
                ...
                before = mem.get_pages(self.key)
                new = mem.get_data(self.key, slice(None)).tolist()  # new = old[:]  # cheap shallow pointer copy in case anything goes wrong.
        ...
        if isinstance(key, int):
            if -self._len-1 < key < self._len:
                ...
                if mem.get_ref_count(page) == 1:
                    ...
                    data = mem.get_data(page.group) # FIXME
            ...
            if key.start == key.stop == None and key.step in (None,1):   # del L[:] == L.clear()
                self._len = mem.create_virtual_dataset(self.key, pages_before=before, pages_after=[])
                ...
                before = mem.get_pages(self.key)
                data = mem.get_data(self.key, slice(None))
                mask = np.ones(shape=data.shape)
    ...
    def __eq__(self, other):
        if isinstance(other, (list,tuple)):
            ...
            if mem.get_pages(self.key)== mem.get_pages(other.key):  # special case.
                return True  
            else:
    ...
    def __imul__(self, other):
        ...
        self._len = mem.create_virtual_dataset(self.key, old_pages, new_pages)
        return self
...
def filter_evaluation_task(table_key, expression, shm_name, shm_index, shm_shape, slice_):
    ...
    if c1 is not None:
        ...
        dset_A = mem.get_data( column_key=column_key, item=slice_)
    else:  # v1 is active:
        dset_A = np.array([v1] * (slice_.stop-slice_.start))
    
    if c2 is not None:
        column_key = columns[c2]
        dset_B = mem.get_data(column_key, slice_)
    else:  # v2 is active:
...
def text_reader(path, newline='\n', text_qualifier=None, delimiter=',', first_row_has_headers=True, 
    ...
    **kwargs are excess arguments that are ignored.
    ...
    with path.open('r', encoding=encoding) as fi:
        ...
        with tqdm(desc=f"splitting {path.name} for multiprocessing", total=newlines, unit="lines") as pbar:
            ...
            if parts:  # any remaining parts at the end of the loop.
                ...
                pbar.update(len(parts))
                parts.clear()
                config.update({"source":str(p), "table_key":mem.new_id('/table')})  ###? FIXME?
                tasks.append( Task( text_reader_task, **{**config, **{"source":str(p), "table_key":mem.new_id('/table'), 'encoding':'utf-8'}} ) )
    ...
    t = None
    for task in tasks:
        tmp = Table.load(key=task.kwargs["table_key"])
        if t is None:
            t = tmp.copy()
        else:
            t += tmp
...
def indexing_task(source_key, destination_key, shm_name_for_sort_index, shape):
    ...
    *used by sort and all join functions.
    ...
    data = mem.get_data(source_key, slice(None)) # --- READ!
